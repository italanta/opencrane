...
OpenCrane consists of three core layers: a **Control Plane API** that manages tenants and policies, an **Operator** that reconciles resources on Kubernetes, **Per-Tenant Pods** running isolated OpenClaw instances with retrieval plugins, and **Company-Wide Information Agents** that gather and index organizational knowledge.

```
┌──────────────────────────────────────────────────────────────────────────────────────────────────┐
│                              Company-Wide Information Gathering Layer                            │
│  Central Agents harvest data from Slack, Teams, email, tickets → Index in Org Knowledge Store   │
└────────────────────────────┬─────────────────────────────────────────────────────────────────────┘
                             │
┌────────────────────────────┴─────────────────────────────────────────────────────────────────────┐
│                           Organizational Knowledge Index                                        │
│  • Departments & team hierarchies  • Projects & ownership  • Shared context  • Company info     │
└────────────┬──────────────────────────────────────────────────────────────────────────┬─────────┘
             │                                                                          │
             │                     ┌─────────────────────────────┐                     │
             │                     │  Control Plane (Express)    │                     │
             │                     │  API: CRUD, Policies, Audit │                     │
             │                     │  Dual-write: CRDs + Postgres│                     │
             │                     └────────────────┬────────────┘                     │
             │                                      │                                  │
             │                     ┌────────────────▼────────────┐                    │
             │                     │   Operator (Kubernetes)     │                    │
             │                     │  Watch Tenant/AccessPolicy  │                    │
             │                     │  Reconcile: RBAC, Storage   │                    │
             │                     └────────────────┬────────────┘                    │
             │                                      │                                  │
    ┌────────┴────────┬──────────────────┬─────────┴──────────┐                      │
    ▼                 ▼                  ▼                     ▼                       │
┌────────────┐  ┌────────────┐   ┌────────────┐   ┌────────────────┐                │
│  jente     │  │   bob      │   │   niels    │   │  info-harvest  │                │
│ OpenClaw   │  │ OpenClaw   │   │ OpenClaw   │   │  Agent (system)│                │
│ (isolated) │  │ (isolated) │   │ (isolated) │   │ (gathers org   │                │
├────────────┤  ├────────────┤   ├────────────┤   │  knowledge)    │                │
│ Private:   │  │ Private:   │   │ Private:   │   ├────────────────┤                │
│ • Drive    │  │ • Drive    │   │ • Drive    │   │ System Private │                │
│ • Secrets  │  │ • Secrets  │   │ • Secrets  │   │ • Drive        │                │
├────────────┤  ├────────────┤   ├────────────┤   │ • Secrets      │                │
...
└────────────┘  └────────────┘   └────────────┘         │                           │
    ...
    └───────────────┼─────────────────┴───────────────────┼───────────────────────────┘
        ...
        ▼                                 ▼                         ▼
    ┌──────────────┐          ┌─────────────────────┐    ┌──────────────────────┐
    │Shared Skills │          │PostgreSQL (Audit)   │    │Org Knowledge Index   │
    │      PVC     │          │ • Tenants           │    │ • Org metadata       │
    │  /shared/    │          │ • Policies          │    │ • Skill registry     │
    │  ├── org/    │          │ • Skills registry   │    │ • Team hierarchy     │
    │  └── team/   │          │ • Token usage       │    │ • Project metadata   │
    └──────────────┘          │ • Audit logs        │    └──────────────────────┘
                              └─────────────────────┘
```
