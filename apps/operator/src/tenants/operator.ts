...
export class TenantOperator
{
  ...
  async reconcileTenant(tenant: Tenant): Promise<void>
  {
    ...
    try
    {
      ...
      // 9. NetworkPolicy — lock the gateway port to the identity-routing gateway proxy
      //    (DOMAIN.T4) so the trusted-proxy auth (CONN.4) can't be abused by other
      //    in-cluster pods. No per-user Ingress is minted any more: every user reaches
      //    their pod through the org's single host, reverse-proxied by the gateway proxy
      //    to this pod's Service. The org host `<org>.<base>` is served by the platform
      //    wildcard Ingress + cert — no per-user record/cert.
      ...
      await this.statusWriter.patchStatus(tenant, crNamespace, {
        ...
        // The user is served at the ORG host (`<org>.<base>` or vanity) via the gateway
        // proxy — no per-user subdomain. Report that as the tenant's ingress host.
