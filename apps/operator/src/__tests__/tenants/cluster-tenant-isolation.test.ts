...
describe("ClusterTenant isolation enforcement (CT.5 reconcile flow)", () =>
{
  ...
  it("serves a ref'd openclaw at the ORG apex host <org>.<base> (no per-user subdomain)", async () =>
  {
    const clusterTenant = _makeClusterTenant("acme", "ct-acme");
    const tenant = _makeTenant("mike", { clusterTenantRef: "acme" });
    ...
    // No per-user Ingress is minted now; the user is served at the org's single host
    // (acme.opencrane.local) via the gateway proxy. The status records the org host.
