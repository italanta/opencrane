...
describe("TenantResourceBuilder", () =>
{
  ...
  it("builds a NetworkPolicy locking the gateway port to the gateway-proxy (CONN.4 / DOMAIN.T4)", () =>
  {
    const tenant = _makeTenant("sarah");
    const config = { ...defaultConfig, gatewayProxyNamespace: "opencrane" };
    const netpol = _BuildGatewayNetworkPolicy(config, tenant, "default");

    expect(netpol.spec?.policyTypes).toEqual(["Ingress"]);
    // Only the gateway-proxy pods (by component label) in the proxy's namespace may
    // reach the gateway port — no other pod can connect and assert an arbitrary
    // X-Forwarded-User. (Per-user Ingresses are retired; the proxy is the sole client.)
    ...
    expect(rule?._from?.[0]?.podSelector?.matchLabels?.["app.kubernetes.io/component"]).toBe("gateway-proxy");
