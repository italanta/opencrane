...
const _PAYLOAD_TYPES = ["mcp-server", "skill-bundle"] as const;
/** Recipient kinds a share may target. */
const _RECIPIENT_TYPES = ["user", "group"] as const;
...
const _COMPILER_PAYLOAD_BY_API: Record<_PayloadTypeStr, GrantCompilerPayloadType> = {
...
const _PRISMA_PAYLOAD_BY_API: Record<_PayloadTypeStr, GrantPayloadType> = {
...
const _PRISMA_SCOPE_BY_API: Record<_ScopeStr, GrantScope> = {
  org: GrantScope.Org,
...
export function sharesRouter(prisma: PrismaClient): Router
{
  ...
  router.post("/", async function _createShare(req: Request, res)
  {
    ...
    const body = (req.body ?? {}) as _CreateShareBody;
    const payloadType = body.payloadType as _PayloadTypeStr;
    const recipientType = body.recipientType as _RecipientTypeStr;
    const scope = (body.scope ?? "personal") as _ScopeStr;
    ...
    // 6. Idempotent: re-sharing the same payload to the same recipient returns the existing
    //    share rather than stacking duplicate grants.
    ...
    const existing = await prisma.grant.findFirst({
      where: { sharedBy: caller, payloadType: _PRISMA_PAYLOAD_BY_API[payloadType], payloadId, subjectType, subjectId: recipientId, access: GrantAccess.Allow },
    });
    if (existing)
    {
  ...
  router.get("/", async function _listShares(req: Request, res)
  {
    const caller = _CallerSubject(req);
    if (!caller)
    {
  ...
  router.delete("/:id", async function _revokeShare(req: Request<{ id: string }>, res)
  {
    const caller = _CallerSubject(req);
    if (!caller)
    {
